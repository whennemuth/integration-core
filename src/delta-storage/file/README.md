...
```typescript
import { FileSystemDeltaStorage } from 'integration-core/delta-storage/file';
...
const previousData = await storage.fetchPreviousInput('client-123');
...
```typescript
import { S3BucketDeltaStorage } from 'integration-core/delta-storage/file';
...
export const handler = async (event: any) => {
  ...
  const previousData = await storage.fetchPreviousInput(event.clientId);
...
describe('DeltaStorage', () => {
  it('should handle large datasets', async () => {
    ...
    const retrieved = await storage.fetchPreviousInput('client');
