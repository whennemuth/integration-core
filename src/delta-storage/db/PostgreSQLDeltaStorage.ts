...
export class PostgreSQLDeltaStorage implements DatabaseDeltaStorage {
   ...
   */
  async storeCurrentData(clientId: string, data: FieldSet[], primaryKeyFields: Set<string>): Promise<any> {
    if (!clientId) {
   ...
   */
  async fetchPreviousData(clientId: string, limitTo?: FieldSet[]): Promise<FieldSet[]> {
    if (!clientId) {
    ...
    if (limitTo && limitTo.length > 0) {
      ...
      const primaryKeyValues = limitTo.map(fs => {
        ...
        const primaryKeyArray = Array.from(fs.fieldValues.map(fv => Object.values(fv)[0]));
    ...
    if (limitTo && limitTo.length > 0) {
      ...
      const fieldNames = firstRecord.fieldValues.map(fv => Object.keys(fv)[0]);
   ...
   */
  async fetchDelta(clientId: string, primaryKeyFields: Set<string>): Promise<DeltaResult> {
    if (!clientId) {
   ...
   */
  async updatePreviousData(clientId: string, newPreviousData: FieldSet[], primaryKeyFields?: Set<string>): Promise<any> {
    if (!clientId) {
    ...
    if (newPreviousData && newPreviousData.length > 0) {
      ...
      return await this.databaseProvider.getDataSource().transaction(async manager => {
        ...
        if (entities.length > 0) {
          // Batch insert new previous data
          for (const entity of entities) {
            await manager.query(`
              INSERT INTO "${tableNames.previous}" (primaryKey, hash, createdAt)
              VALUES (?, ?, ?)
