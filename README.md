...
graph TD
    ...
    B -->|convertRawToInput| C[Input Format]
...
The same flow can also be viewed as a sequence of interactions between components:
...
sequenceDiagram
    ...
    participant IU as Input Utils

    Note over DS,IU: Integration Cycle Start
    
    DS->>DS: fetchRaw()
    DS->>IP: convertRawToInput(rawData)
    IP->>IP: parse() - validate & hash
    IP-->>DE: validated fieldSets
    
    ST->>DE: fetchPreviousData()
    DE->>DE: computeDelta(current, previous)
    DE-->>DT: delta results
    
    alt Has Changes
        DT->>DT: pushAll(added, updated, removed)
        DT-->>IU: BatchPushResult
        
        alt Push Failures
            IU->>IU: restorePreviousHashesForFailures()
            Note right of IU: Restore hashes for<br/>failed records
        end
        
        IU->>ST: updatePreviousData(processedFieldSets)
    else No Changes
        Note over DE,ST: Skip push, update baseline
        DE->>ST: updatePreviousData(currentFieldSets)
    end
    
    Note over DS,IU: Integration Cycle Complete
```
