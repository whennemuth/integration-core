...
describe('FileSystemDeltaStorage', () => {
  ...
  describe('fetchPreviousInput', () => {
    it('should successfully read field sets from NDJSON file', async () => {
      ...
      const result = await storage.fetchPreviousData(testClientId);
    ...
    it('should handle malformed JSON lines gracefully', async () => {
      ...
      const fetchPromise = storage.fetchPreviousData(testClientId);
    ...
    it('should require clientId', async () => {
      await expect(storage.fetchPreviousData(''))
        .rejects.toThrow('clientId is required for fetchPreviousInput');
    });
  });

  describe('updatePreviousData', () => {
    it('should successfully update previous data when new data provided', async () => {
      const result = await storage.updatePreviousData(testClientId, testFieldSets);
    ...
    it('should handle case when no new data provided (cleanup previous)', async () => {
      ...
      const result = await storage.updatePreviousData(testClientId);
    ...
    it('should require clientId', async () => {
      await expect(storage.updatePreviousData(''))
  ...
  describe('Integration workflow', () => {
    it('should handle complete workflow with multiple data updates', async () => {
      ...
      await storage.updatePreviousData(testClientId, [testFieldSets[0]]);
      
      // Verify we can fetch the previous data
      const previousData = await storage.fetchPreviousData(testClientId);
      expect(previousData).toEqual([testFieldSets[0]]);
      
      // Second cycle: update with new data
      await storage.updatePreviousData(testClientId, [testFieldSets[1]]);
      
      // Now previous should be the second data
      const newPreviousData = await storage.fetchPreviousData(testClientId);
    ...
    it('should handle large datasets efficiently', async () => {
      ...
      const updateResult = await storage.updatePreviousData('large-client', largeData);
      expect(updateResult.status).toBe('success');

      // Fetch and verify
      const fetchedData = await storage.fetchPreviousData('large-client');
