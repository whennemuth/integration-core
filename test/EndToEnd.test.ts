...
describe('EndToEnd', () => {
  ...
  const getDeltaResult = async (testScenario: TestScenario): Promise<{ deltaResult: DeltaResult, pushResult: PushResult }> => {
    ...
    const deltaStorage = testScenario === TestScenario.BRUTE_FORCE 
    ...
    if(testScenario === TestScenario.BRUTE_FORCE) {
      ...
      await dbDeltaStorage.storeCurrentData(clientId, currentParsedInput.fieldSets);

      const deltaResult = await dbDeltaStorage.fetchDelta(clientId);
