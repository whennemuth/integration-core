...
describe('InputUtilsDecorator', () => {
  ...
  describe('getReducedFieldSets', () => {
    it('should return field sets containing only primary key fields and hash', () => {
      const result = inputUtils.getReducedFieldSets();
    ...
    it('should return empty array when no field sets exist', () => {
      ...
      const result = emptyUtils.getReducedFieldSets();
    ...
    it('should handle field sets with no primary key fields', () => {
      ...
      const result = utilsWithoutPKValues.getReducedFieldSets();
  ...
  describe('restorePreviousHashesForFailures', () => {
    let currentInputFieldSets: FieldSet[];
    let previousInputFieldSets: FieldSet[];

    beforeEach(() => {
      currentInputFieldSets = [
        {
    ...
    it('should restore previous hashes for failed records', () => {
      ...
      expect(currentInputFieldSets[0].hash).toBe('oldHash1'); // id: 1 failed
      expect(currentInputFieldSets[1].hash).toBe('newHash2'); // id: 2 succeeded, keeps new hash
      expect(currentInputFieldSets[2].hash).toBe('oldHash3'); // id: 3 failed
    });
    ...
    it('should do nothing when there are no failures', () => {
      ...
      const originalHashes = currentInputFieldSets.map(fs => fs.hash);
      ...
      currentInputFieldSets.forEach((fs, index) => {
        expect(fs.hash).toBe(originalHashes[index]);
      });
    });

    it('should do nothing when failures array is undefined', () => {
      const pushResult: BatchPushResult = {
        status: BatchStatus.SUCCESS,
    ...
    it('should handle failures for records not found in previous input', () => {
      ...
      // All hashes should remain unchanged since the failed record doesn't exist
      currentInputFieldSets.forEach((fs, index) => {
        expect(fs.hash).toBe(originalHashes[index]);
      });
    });
    ...
    it('should handle failures for records not found in current input', () => {
      ...
      const reducedCurrentInput = currentInputFieldSets.slice(1);
      const originalHashes = reducedCurrentInput.map(fs => fs.hash);

      inputUtils.restorePreviousHashesForFailures({
        currentInputFieldSets: reducedCurrentInput,
        previousInputFieldSets,
        pushResult
      });
    ...
    it('should handle composite primary keys correctly', () => {
      ...
      expect(currentInputFieldSets[0].hash).toBe('newHash1'); // Different record
      expect(currentInputFieldSets[1].hash).toBe('oldHash2'); // Matching composite key - restored
      expect(currentInputFieldSets[2].hash).toBe('newHash3'); // Different record
    });
    ...
    it('should handle single primary key fields', () => {
      ...
      singlePKUtils.restorePreviousHashesForFailures({
        currentInputFieldSets: singlePKCurrent,
        previousInputFieldSets: singlePKPrevious,
        pushResult
      });
    ...
    it('should restore previous hashes for records with validation issues (missing hashes)', () => {
      ...
      inputUtils.restorePreviousHashesForFailures({
        currentInputFieldSets: currentWithValidationIssues,
        previousInputFieldSets,
        pushResult
      });
    ...
    it('should restore hashes for both push failures and validation issues', () => {
      ...
      inputUtils.restorePreviousHashesForFailures({
        currentInputFieldSets: currentWithMixedIssues,
        previousInputFieldSets,
        pushResult
      });
    ...
    it('should not restore hashes for records with validation messages that already have hashes', () => {
      ...
      inputUtils.restorePreviousHashesForFailures({
        currentInputFieldSets: currentWithHashedValidationIssues,
        previousInputFieldSets,
        pushResult
      });
    ...
    it('should handle records with validation issues not found in previous input', () => {
      ...
      inputUtils.restorePreviousHashesForFailures({
        currentInputFieldSets: currentWithNewInvalidRecord,
        previousInputFieldSets,
        pushResult
      });

      // New invalid record should remain without hash (no previous record to restore from)
      expect(currentWithNewInvalidRecord[0].hash).toBeUndefined();
      
      // Existing valid record should keep its hash
      expect(currentWithNewInvalidRecord[1].hash).toBe('newHash1');
    });
    ...
    it('should handle empty validation messages map correctly', () => {
      ...
      inputUtils.restorePreviousHashesForFailures({
        currentInputFieldSets: currentWithEmptyValidationMessages,
        previousInputFieldSets,
        pushResult
      });
