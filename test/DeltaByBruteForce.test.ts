import { DeltaParms } from "../src/DeltaTypes";
...
describe('DeltaByBruteForce', () => {
  ...
  it('should compute the delta between two data sets with single field primary key for determining updated fields', 
    ...
    const deltaParms = {
      fishOutTheUpdates: (addedRecords, removedRecords) => {
        return fishOutUpdatedRecordsByPK(addedRecords, removedRecords, singleFieldPK);
      },
      data: {
        previous: [
    ...
    expect(newRecords.map(r => r.fieldValues[0]['id'])).toEqual(expect.arrayContaining([4, 5]));

    expect(changedRecords).toHaveLength(1);
    expect(changedRecords![0].fieldValues[0]['id']).toBe(3);

    expect(removedRecords).toHaveLength(1);
    expect(removedRecords[0].fieldValues[0]['id']).toBe(1);
  });
  ...
  it('should compute the delta between two data sets with composite primary key for determining updated fields', 
    ...
    const deltaParms = {
      fishOutTheUpdates: (addedRecords, removedRecords) => {
        return fishOutUpdatedRecordsByPK(addedRecords, removedRecords, compositePK);
      },
      data: {
        previous: [
    ...
    expect(newRecords.map(r => r.fieldValues[0]['id'])).toEqual(expect.arrayContaining([4, 5, 6]));
    expect(newRecords.map(r => r.fieldValues[0]['username'])).toEqual(expect.arrayContaining(['diana', 'jane', 'bob']));
    expect(newRecords.map(r => r.fieldValues[0]['name'])).toEqual(expect.arrayContaining(['Diana', 'Jane', 'Bobby']));

    expect(changedRecords).toHaveLength(1);
    expect(changedRecords![0].fieldValues[0]['id']).toBe(3);
    expect(changedRecords![0].fieldValues[0]['username']).toBe('charlie');
    expect(changedRecords![0].fieldValues[0]['name']).toBe('Charlie Updated');

    expect(removedRecords).toHaveLength(2);
    expect(removedRecords.map(r => r.fieldValues[0]['id'])).toEqual(expect.arrayContaining([1, 6]));
    expect(removedRecords.map(r => r.fieldValues[0]['username'])).toEqual(expect.arrayContaining(['alice', 'jane']));
    expect(removedRecords.map(r => r.fieldValues[0]['name'])).toEqual(expect.arrayContaining(['Alice', 'Jane']));
  });
});