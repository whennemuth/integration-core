...
import { StreamProvider } from '../src/delta-storage/file/StreamProvider';
...
describe('S3BucketDeltaStorage', () => {
  ...
  describe('fetchPreviousInput', () => {
    it('should throw error for empty clientId', async () => {
      await expect(deltaStorage.fetchPreviousData('')).rejects.toThrow(
        'clientId is required for fetchPreviousInput'
      );
    });

    it('should return empty array when no previous data exists', async () => {
      const result = await deltaStorage.fetchPreviousData('client1');
    ...
    it('should return previous data when it exists', async () => {
      ...
      await deltaStorage.updatePreviousData('client1', testData);
  ...
  describe('updatePreviousData', () => {
    it('should throw error for empty clientId', async () => {
      await expect(deltaStorage.updatePreviousData('', [])).rejects.toThrow(
        'clientId is required for updatePreviousData'
      );
    });

    it('should handle case when no new data provided (cleanup previous)', async () => {
      const result = await deltaStorage.updatePreviousData('client1', []);
    ...
    it('should update previous data successfully with new data', async () => {
      ...
      const result = await deltaStorage.updatePreviousData('client1', testData);
      ...
      const fetchedData = await deltaStorage.fetchPreviousData('client1');
  ...
  describe('Integration workflow', () => {
    it('should handle complete workflow with multiple clients', async () => {
      ...
      await deltaStorage.updatePreviousData('client1', client1Data);
      await deltaStorage.updatePreviousData('client2', client2Data);
    ...
    it('should handle large datasets efficiently', async () => {
      ...
      const updateResult = await deltaStorage.updatePreviousData('large-client', largeData);
      expect(updateResult.status).toBe('success');

      // Fetch and verify all data
      const fetchedData = await deltaStorage.fetchPreviousData('large-client');
